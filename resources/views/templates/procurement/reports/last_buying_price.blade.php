{{-- last_buying_price.blade.php --}}
@extends('templates.app')

@section('content')
@include('templates.storage_supplies.partials.active_subdepartment_bar')
<div class="settings-panel-head"><h2>Last Buying Price Report</h2></div>

<form id="reportFilterForm" class="row g-3 mb-3" data-endpoint="{{ route('procurement.reports.last_buying_price_data') }}">
    <div class="col-md-4">
        <label class="form-label">Item Category</label>
        <select name="item_category_id" class="form-select">
            <option value="">All</option>
            @foreach($itemCategories as $c)<option value="{{ $c->id }}">{{ $c->name }}</option>@endforeach
        </select>
    </div>
