...
<div class="settings-panel-head"><h2>Fuel by Petrol Station</h2></div>

<form id="reportFilterForm" class="row g-3 mb-3" data-endpoint="{{ route('fleet.reports.fuel_by_station_data') }}">
    ...
    <div class="col-md-4">
        <label class="form-label">Petrol Station</label>
        <select name="fuel_source_id" class="form-select">
            <option value="">All</option>
            @foreach($fuelSources as $s)<option value="{{ $s->id }}">{{ $s->name }}</option>@endforeach
        </select>
    </div>
    <div class="col-md-4">
        <label class="form-label">Type of Order</label>
        <select name="type" class="form-select">
            <option value="itinerary_leg">Itineraries & Legs</option>
