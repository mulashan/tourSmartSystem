...
<body>
    ...
    <div class="title">RETURN INWARD</div>

    <div class="meta">
        <div><label>Document Number</label> : {{ $return->id }}</div>
        <div><label>Return Date</label> : {{ $return->return_date }}</div>
        <div><label>Posted By</label> : {{ $return->postedBy->name ?? '—' }}</div>
        <div><label>Store Returning</label> : {{ $return->fromSubdepartment->Subdepartment_Name ?? '—' }}</div>
        <div><label>Store Receiving</label> : {{ $return->toSubdepartment->Subdepartment_Name ?? '—' }}</div>
        ...
        @if($return->received_at)
            <div><label>Received By</label> : {{ $return->receivedBy->name ?? '—' }} on {{ $return->received_at }}</div>
        @else
