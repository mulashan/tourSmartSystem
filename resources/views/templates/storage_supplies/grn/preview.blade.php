...
<head>
    <meta charset="UTF-8">
    <title>GRN - {{ $grn->Grn_Purchase_Order_ID }}</title>
    <style>
...
<body>
    ...
    <div class="title">GOODS RECEIVING NOTE</div>

    <div class="meta">
        <div><label>GRN No.</label> : {{ $grn->Grn_Purchase_Order_ID }}</div>
        <div><label>LPO No.</label> : {{ $grn->local_purchase_order_id }}</div>
        <div><label>Store Requesting</label> : {{ $grn->subdepartment->Subdepartment_Name ?? '—' }}</div>
        <div><label>Supplier</label> : {{ $grn->supplier->supplier_name ?? '—' }}</div>
        <div><label>Delivery Note Number</label> : {{ $grn->Delivery_Note_Number }}</div>
        <div><label>Invoice Number</label> : {{ $grn->Invoice_Number }}</div>
        <div><label>Delivery Date</label> : {{ $grn->Delivery_Date }}</div>
        <div><label>Delivery Person</label> : {{ $grn->Delivery_Person }}</div>
