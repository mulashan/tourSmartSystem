    ...
    <div class="card-body">
        <div class="row g-3">
            ...
            <div class="col-md-3">
                ...
                <input type="date" id="transactionDate" class="form-control" value="{{ $return->transaction_date }}" required>
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <input type="text" class="form-control" value="{{ $return->subdepartment->Subdepartment_Name ?? '—' }}" disabled>
            </div>
            <div class="col-md-6">
                <label class="form-label">Receiving Supplier</label>
                <input type="text" class="form-control" value="{{ $return->supplier->supplier_name ?? '—' }}" disabled>
...
<div class="text-end mt-3">
    <a href="{{ route('storage_supplies.return_outward.new') }}" class="btn btn-secondary">Cancel</a>
...
<script>
    window.returnOutwardRoutes = {
        update: '{{ route("storage_supplies.return_outward.update", $return->id) }}',
        draftList: '{{ route("storage_supplies.return_outward.new") }}',
    };
    window.returnOutwardExistingLines = @json($existingLines);
</script>
<script src="{{ asset('assets/js/item-picker.js') }}"></script>
<script src="{{ asset('assets/js/return-outward-edit.js') }}"></script>
@endsection