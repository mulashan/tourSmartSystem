(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        $('#storeRequestingLabel').val($('.settings-panel-head strong').first().parent().text().replace('Sub Department', '').trim());

        let issuingSubdepartmentId = null;
        let lines = []; // { id, name, uom, balance, quantity }
        let searchTimer;

        $('#js-lock-header').on('click', function () {
            const description = $('#description').val().trim();
            const storeId = $('#issuingSubdepartmentId').val();

            if (! description || ! storeId) {
                Swal.fire({ icon: 'warning', title: 'Missing fields', text: 'Please provide a Description and select a Store Issuing before continuing.' });
                return;
            }

            issuingSubdepartmentId = storeId;
            $('#description, #issuingSubdepartmentId, #js-lock-header').prop('disabled', true);
        ...
        $('#js-save-requisition').on('click', function () {
            ...
            $.ajax({
                url: window.requisitionRoutes.store,
                method: 'POST',
                data: {
                    issuing_subdepartment_id: issuingSubdepartmentId,
