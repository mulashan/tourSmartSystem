(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        let items = window.grnOpenBalanceExistingItems || [];
        ...
        function submitPayload(submit) {
            ...
            $.ajax({
                url: window.grnOpenBalanceRoutes.update,
                method: 'POST',
                data: {
                ...
                .done(() => {
                    Swal.fire({ icon: 'success', title: submit ? 'Submitted for approval' : 'Saved as draft', timer: 1200, showConfirmButton: false })
                        .then(() => { window.location.href = window.grnOpenBalanceRoutes.newList; });
                })
                .fail(xhr => {
        ...
        $('#js-save-draft').on('click', () => submitPayload(false));
        $('#js-save-submit').on('click', () => submitPayload(true));
