(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        let items = []; // { id, name, uom, remarks, batches: [] }
        ...
        $('#deliveryNoteNumber, #invoiceNumber, #deliveryDate').on('input change', refreshSubmitButtonState);
        ...
        $('#js-submit-grn').on('click', function () {
            if (! refreshSubmitButtonState()) {
                Swal.fire({ icon: 'warning', title: 'Not ready yet', text: 'Fill in all required header fields and add at least one batch for every item.' });
                return;
            }
            ...
            formData.append('purchase_description', $('#purchaseDescription').val());
            ...
            $.ajax({ url: window.grnWithoutPoRoutes.store, method: 'POST', data: formData, processData: false, contentType: false })
                .done(() => {
