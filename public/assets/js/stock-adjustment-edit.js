(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        const reason = window.stockAdjustmentReason;
        let addItems = window.stockAdjustmentExistingAddItems || [];
        let deductItems = window.stockAdjustmentExistingDeductItems || [];
        let searchTimer;
        ...
        function buildPayload(submit) {
            ...
            if (reason === 'add_stock_balance') {
                ...
                return {
                    adjustment_date: $('#adjustmentDate').val(), description: $('#description').val(), submit: submit ? 1 : 0,
        ...
        function save(submit) {
            ...
            $.post(window.stockAdjustmentRoutes.update, payload)
                .done(() => {
                    Swal.fire({ icon: 'success', title: submit ? 'Submitted for approval' : 'Saved', timer: 1200, showConfirmButton: false })
