(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        $('#storeReturningLabel').val($('.settings-panel-head strong').first().parent().text().replace('Sub Department', '').trim());

        let toSubdepartmentId = null;
        let lines = [];

        $('#js-lock-header').on('click', function () {
            const returnDate = $('#returnDate').val();
            const description = $('#description').val().trim();
            const storeId = $('#toSubdepartmentId').val();

            if (! returnDate || ! description || ! storeId) {
                Swal.fire({ icon: 'warning', title: 'Missing fields', text: 'Please provide Return Date, Description, and select a Store Receiving before continuing.' });
                return;
            }

            toSubdepartmentId = storeId;
            $('#returnDate, #description, #toSubdepartmentId, #js-lock-header').prop('disabled', true);
        ...
        $('#js-cancel-return').on('click', function () {
            window.location.href = window.returnRoutes.draftList;
        });
        ...
        function savePayload() {
            ...
            return $.post(window.returnRoutes.store, {
                to_subdepartment_id: toSubdepartmentId,
            ...
            savePayload()
                .then(() => {
                    Swal.fire({ icon: 'success', title: 'Saved as draft', timer: 1200, showConfirmButton: false })
                        .then(() => { window.location.href = window.returnRoutes.draftList; });
                })
                .catch(xhr => {
                    if (! xhr) return; // rejected before request fired (validation message already shown)
        ...
        $('#js-save-submit-return').on('click', function () {
            savePayload()
                .then(response => {
                    return $.post(`/storage-supplies/return/${response.id}/submit`);
                })
                .then(() => {
