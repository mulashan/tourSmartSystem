(function whenJQueryReady(fn) {
    ...
    $(function () {
        ...
        function refreshSubmitButtonState() {
            const headerComplete = $('#returnDate').val() && $('#reason').val().trim();
            const allHaveBatches = items.length > 0 && items.every(item => item.batches.length > 0);
            const ready = headerComplete && allHaveBatches;
            $('#js-submit-grn').prop('disabled', ! ready);
            return ready;
        }

        $('#returnDate, #reason').on('input change', refreshSubmitButtonState);
        ...
        function renderItems() {
            ...
            items.forEach((item, i) => {
                ...
                $body.append(`
                    <tr data-index="${i}">
                        <td>${i + 1}</td><td>${item.name}</td><td>${item.uom}</td><td>${totalQty}</td>
        ...
        $('#js-submit-grn').on('click', function () {
            if (! refreshSubmitButtonState()) {
                Swal.fire({ icon: 'warning', title: 'Not ready yet', text: 'Complete the header fields and add at least one batch for every item.' });
                return;
            }

            const $btn = $(this).prop('disabled', true).html('<span class="spinner-border spinner-border-sm"></span> Submitting...');

            $.post(window.returnInwardRoutes.store, {
                return_date: $('#returnDate').val(),
                returned_from: $('#returnedFrom').val(),
                reason: $('#reason').val(),
                items: items.map(item => ({ item_id: item.id, remarks: item.remarks, batches: item.batches })),
            })
                .done(() => {
                    Swal.fire({ icon: 'success', title: 'Submitted for approval', timer: 1200, showConfirmButton: false })
                        .then(() => { window.location.href = window.returnInwardRoutes.approveList; });
                })
                .fail(xhr => {
